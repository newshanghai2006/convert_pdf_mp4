...
- **AI 旁白失败可定位重试**：失败片段会在编辑区标记并保留原始文字，可只重试指定片段，不必重复生成全部旁白。
- **旁白可编辑**：每段旁白单独编辑，也可**逐段单独设定时长**；提取完成后每段旁白旁会显示对应页面缩略图。
...
- **兼容性**：输出 MP4 带 `+faststart`，下载后各播放器都能直接打开；成品完成后还可下载最终封面 PNG 图片。
...
1. **选 PDF + 基本参数**：每片段时长默认 8 秒，默认按解说词自动延长；AI 自动生成旁白及去除提取文字中的换行和空格默认开启。选择页码区域会显示原 PDF 总页数。也可设置页数、封面时长、页码范围、OCR 引擎与识别语言；需要时勾选 AI OCR 并填写视觉模型配置。
