...
- **AI 配音**：微软 [edge-tts](https://github.com/rany2/edge-tts) 在线合成，6 种中文声线 + 语速可调；网络失败有清晰提示与降级。
...
- **字幕**：可生成 **SRT 文件**，或把字幕**烧进画面**（硬字幕，内置楷体渲染）。
...
启动时会打印依赖自检；随后浏览器打开 **http://127.0.0.1:5005** 即可使用（仅本机访问）。
...
- **仅本机**：默认监听 `127.0.0.1:5005`。如需局域网访问，自行修改 `app.py` 的 `host`（注意安全）。
- 本项目为个人/学习用途的自动化小工具。
